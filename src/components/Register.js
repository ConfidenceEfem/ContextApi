...
const Register = ()=>{
    ...
    const {register, handleSubmit,formState:{error},reset} = useForm({resolver: yupResolver(schema)})
    ...
    return(
        <Container>
            <Wrapper onSubmit={done}>
                ...
                <Error><span>{error?.username?.message}</span></Error>
                <Input placeholder="Enter username"
                {...register("username")}/>
                <Error><span>{error?.email?.message}</span></Error>
                <Input placeholder="Enter email"
                {...register("email")}/>
                <Error><span>{error?.password?.message}</span></Error>
                <Input placeholder="Enter password"
                {...register("password")}/>
                <Error><span>{error?.confirm?.message}</span></Error>
